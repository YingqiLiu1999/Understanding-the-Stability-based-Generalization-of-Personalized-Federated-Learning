...
class Client:
    ...
    def train(self, w_global,w_local,round):
        if self.args.dataset == "emnist":
            ...
            body_params = {name: copy.deepcopy(w_global[name]) for name in w_global if 'classifier'  not in name}
            head_params = {name: copy.deepcopy(w_local[name]) for name in w_local if 'classifier' in name}
        
        self.model_trainer.set_model_params(body_params)
        self.model_trainer.set_model_params(head_params)
        self.model_trainer.set_id(self.client_idx)
        test_local_metrics,train_local_metrics = self.model_trainer.train \
            (self.local_training_data, self.local_test_data,self.device, self.args, round)
        
        weights = self.model_trainer.get_model_params()
        return  weights,test_local_metrics,train_local_metrics#,training_flops,num_comm_params
