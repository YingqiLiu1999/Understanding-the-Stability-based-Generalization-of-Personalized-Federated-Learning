...
def partition_data( datadir, partition, n_nets, alpha, logger,idex = 0):
    ...
    if partition == 'n_cls':
        ...
        while np.sum(clnt_data_list) != 0:
            ...
            while True:
                ...
                # Redraw class label if trn_y is out of that class
                if cls_amount[cls_label] <= 0:
                    cls_amount[cls_label] = np.random.randint(0, len(idx_list[cls_label])) #这个类别没有了就去补另一个类别
                    continue
                cls_amount[cls_label] -= 1
                net_dataidx_map[curr_clnt].append(idx_list[cls_label][cls_amount[cls_label]])
                break
    ...
    if idex > 0:
        for clientidx in net_dataidx_map:
            net_dataidx_map[clientidx].pop(idex)
...
def load_partition_data_cifar100( data_dir, partition_method, partition_alpha, client_number, batch_size, logger,idex=0):
    X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts = partition_data(
                                                                                             data_dir,
                                                                                             partition_method,
                                                                                             client_number,
                                                                                             partition_alpha, logger,idex)
    # get local dataset
